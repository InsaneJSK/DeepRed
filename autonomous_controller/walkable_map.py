...
class RomPassability:
    def __init__(self, pokered_root):
        ...
        self.dims = {name: (int(w), int(h)) for name, w, h in re.findall(
            r'map_const\s+(\w+),\s*(\d+),\s*(\d+)', constants)}
        self.headers = {}
        ...
        self.blocksets = {norm(k): v for k, v in self._binary_labels(
            self.root / 'gfx/tilesets.asm', '_Block').items()}
        ...
        self.pairs = {}
        for tileset, a, b in re.findall(r'db\s+(\w+),\s*\$(\w+),\s*\$(\w+)',
                                        pairs.split('TilePairCollisionsWater')[0]):
    ...
    def tiles(self, name):
        ...
        return tuple(tuple(definitions[blocks[(y//2)*w + x//2]*16
                                           + (y % 2)*8 + (x % 2)*2 + 4]
    ...
    def can_step(self, name, x, y, nx, ny):
        if abs(nx-x) + abs(ny-y) != 1 or not self.is_passable(name, nx, ny):
            return False
        tileset = self.headers.get(name, ('', ''))[1]
        return frozenset((self.tile(name, x, y), self.tile(name, nx, ny))) not in self.pairs.get(tileset, set())
