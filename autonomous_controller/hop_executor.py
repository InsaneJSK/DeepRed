    ...
    def _execute_warp_hop(self, warp, dst_map):
        ...
        for _, direction, approach in sorted(candidates):
            if self.navigate_to_tile(*approach, forbidden_tiles=forbidden):
                ...
                if (self._map_id() == origin and self._pos() == (wx, wy)
                        and not self.interrupt.was_displaced):
                    name = self._map_name()
                    outward = ('down' if wy == self.rom_pass.map_height(name)-1
                               else 'up' if wy == 0
                               else 'right' if wx == self.rom_pass.map_width(name)-1
                               else 'left' if wx == 0 else direction)
                    self._step(outward)
