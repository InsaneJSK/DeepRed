...
class TerrainTests(unittest.TestCase):
    ...
    def test_room_dimensions_are_player_steps(self):
        self.assertEqual((self.terrain.map_width('REDS_HOUSE_2F'),
                          self.terrain.map_height('REDS_HOUSE_2F')), (8, 8))
    ...
    def test_unknown_and_outside_are_blocked(self):
        for name, x, y in [('MISSING', 0, 0), ('REDS_HOUSE_2F', -1, 3),
                           ('REDS_HOUSE_2F', 8, 3), ('REDS_HOUSE_2F', 3, 8)]:
            self.assertFalse(self.terrain.is_passable(name, x, y))

    def test_route_one_requires_real_detours(self):
        terrain = self.terrain
        path = astar(10, 35, 11, 0, lambda x, y: terrain.is_passable('ROUTE_1', x, y),
...
class SearchTests(unittest.TestCase):
    ...
    def test_directional_edges_and_forbidden_warp_detour(self):
        allowed = lambda x, y: 0 <= x < 3 and 0 <= y < 2 and (x, y) != (1, 0)
        path = astar(0, 0, 2, 0, allowed)
        self.assertEqual(path, ['down', 'right', 'right', 'up'])
        self.assertIsNone(astar(0, 0, 2, 0, allowed,
                               can_step=lambda x, y, nx, ny: not (x == 0 and nx == 1)))
...
class ObjectTests(unittest.TestCase):
    ...
    def test_unreachable_navigation_stops_after_bounded_waits(self):
        class Blocked(NavAstar):
            def _map_id(self): return 1
            def _pos(self): return (0, 0)
            def _plan_path(self, *args): return None
        nav = Blocked()
        ticks = []
