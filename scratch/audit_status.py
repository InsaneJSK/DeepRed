...
for state in [
    "saves/in-room-start.state",
    "saves/oak-room-battle.state",
    "Pokemon_Red/Red.gb.state",
]:
    ...
    try:
        ...
        print(
            ...
            json.dumps(
                dict(
                    ...
                    sprites=sprites,
                    party=[v.get("species_name") for v in gs.party_pokemon],
                )
            ),
            flush=True,
        )
        if state == "saves/in-room-start.state":
            ...
            c = AutonomousController(wrapped, gs)
